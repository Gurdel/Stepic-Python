...
line_to_encrypt, line_to_decrypt = input(), input()

dct_encr = {}
dct_decr = {}
for i in range(len(alph1)):
    dct_encr[alph1[i]] = alph2[i]
    dct_decr[alph2[i]] = alph1[i]

encrypted = ''
for c in line_to_encrypt:
    encrypted += dct_encr[c]

decrypted = ''
for c in line_to_decrypt:
    decrypted += dct_decr[c]
...
на второй строке — символы конечного алфавита, после чего идёт строка, которую нужно зашифровать переданным
