dct = {'запад':0, 'восток':0, 'север':0, 'юг':0}

for i in range(int(input())):
    buf = input().split()
    dct[buf[0]] += int(buf[1])

print(dct['восток'] - dct['запад'], dct['север'] - dct['юг'])
...
Программе подаётся на вход число команд nn, которые нужно выполнить черепашке, после
 чего nn строк с самими командами. Вывести нужно два числа в одну строку: первую и вторую 
